...
# maxdelay is set to $watchdogtime in nanoseconds divided by 1000 since we are sending the alive singnal every 100.000 inserts but the delay is done every 100 inserts. The factor 0.9 adds in some tolerance to avaid watchdog is killing service because delay for inserts is to high! This might happen if the 1ast 100.000 inserts are done in less than 2 seconds...
                                                    ...
                                                    if(defined ($timeserieshash{$timeattribute})) {
                                                        if($timeattribute eq "timeInSeconds") {
                                                            ...
                                                            if(defined $perfstats{$fabric}{$switch}{$slot}{$port}{$metrictouse}{$metrictime}) {
                                                                $log->warn("Collision in Metric: ".$metrictouse." at time ".$metrictime." for key ".$targetkey);
                                                            }
...
sub toGraphite {
    ...
    # every 100.000 inserts we will check how long it takes for 100.000 in objects to insert. The delay will be adjusted based on this result compared to the maximum amount of metrics that should be imported.
    if($socketcnt>=100000) {
        alive();
