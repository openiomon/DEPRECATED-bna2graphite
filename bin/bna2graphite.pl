...
sub registerservice {
	...
	if($bnaserver eq 'ALL') {
		...
		if(-f $servicefile) {
			console("There is allready a service with the name bna2graphite-".$bnaservers{$bnaserver}{'shortname'}." registerd. You can either start, stop or restart the service. For updates to servicefile please deregister an register again.");
		} else {
...
sub service {
	...
	if($bnaserver eq 'ALL') {
		...
		if(!-f '/usr/lib/systemd/system/bna2graphite-'.$bnaservers{$bnaserver}{'shortname'}.'.service'){
			console("\tService cannot be found for storagesystem ".$bnaserver.". Please register service or correct defined storagesystem or verify configuration file!");
			return(0);
		}
